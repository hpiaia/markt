...
const MessageFromBot: FC<{message: Message}> = ({ message }) => (
  <div className="mr-auto my-2 flex items-center">
    ...
    <div className="bg-gradient-to-r from-indigo-600 to-purple-600 px-4 py-2 rounded-lg text-white shadow-lg">{message.text}</div>
  </div>
);
...
const MessageReceived: FC<{message: Message}> = ({ message }) => (
  <div className="mr-auto my-2 flex items-center">
    ...
    <div className=" bg-white px-4 py-2 rounded-lg text-gray-600 shadow-lg">{message.text}</div>
  </div>
);

const MessageSent: FC<{message: Message}> = ({ message }) => (
  <div className="ml-auto my-2">
    <div className="bg-indigo-600 px-4 py-2 rounded-lg text-white shadow-lg">{message.text}</div>
  </div>
);
